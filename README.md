...
- **時間単位のログ保存**: 作業コンテキストを JSONL 形式で時間ごとのファイルに自動保存
- **1時間ごとの自動要約**: 毎時0分に前の1時間のログを自動要約（コンテキスト制限を回避）
- **日報自動生成**: Ollama または OpenAI を使って蓄積された要約またはログから日報を自動作成
...
起動すると以下の処理が自動で行われます：
- ウィンドウ切り替え時のキャプチャ・OCR・ログ保存
- 30秒ごとの定期キャプチャ
- **1時間ごとのログ自動要約**（毎時0分に前の1時間のログを要約）
...
└── summaries/
    └── 2024-12-24/
        ├── summary_09.md      # 09:00-10:00 の要約（自動生成）
        ├── summary_10.md      # 10:00-11:00 の要約（自動生成）
        └── ...
```

### ログの要約（手動）

通常は1時間ごとに自動で要約されますが、手動で要約することもできます。
...
蓄積された要約またはログから日報を生成できます。**デフォルトで未要約のログは自動的に要約されます。**
...
# スクリプトで実行（Ollama 起動チェック付き、自動要約付き）
./scripts/report.sh
...
# 自動要約をスキップして日報を生成
./scripts/report.sh --no-auto-summarize
